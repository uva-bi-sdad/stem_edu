# initial pass at making sankey diagrams with ipums data
...
# sankey of womens' edu level, degree field stem/not-stem, and occupation type  --------------------------------------------------------------------------------------------------------------
...
links <- as.data.frame(table(as.character(female$educd_red),
                             as.character(female$occVT)), stringsAsFactors = FALSE)
...
sankeyNetwork(Links = sankey_data$links, Nodes = sankey_data$nodes, Source = "source",
              ...
              units = "freq", fontSize = 16, nodeWidth = 30) %>% saveNetwork('ipums_women_deglevel_3.selected.occupations.html')

# sankey - focus on 3 careers only --------------------------------------------------------------------------------------------------------------
...
sankeyNetwork(Links = sankey_data$links,
              ...
              units = "freq",
              fontSize = 16,
              nodeWidth = 30) %>% saveNetwork(file = 'ipums_women_degtype_only3selected.occs.html')
