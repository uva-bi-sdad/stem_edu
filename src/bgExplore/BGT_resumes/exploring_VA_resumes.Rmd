...
eligiblejobs <- jobs %>% 
  mutate(Start = lubridate::ymd(jobISOStartDate), 
         End = lubridate::ymd(jobISOEndDate), 
         JobDuration = ((End - Start)),
         order_val = as.numeric(Start)) %>% 
  filter(!is.na(Start)) 

tenurecalc <- eligiblejobs %>%
  dt_select(BGTResId, JobDuration) %>% dt_summarise(tenure = sum(JobDuration), by = BGTResId)

tenureportion <- eligiblejobs %>% 
  left_join(tenurecalc, by = "BGTResId") %>% 
  mutate(tenureportion = as.integer(100*(as.numeric(JobDuration)/as.numeric(tenure)))) %>% 
  group_by(BGTResId) %>% 
  mutate(JobOrder =  order(order_val, decreasing=TRUE),
         TotalJobs = max(JobOrder),
         trajectory_percentage = as.integer(100*(1 - ((TotalJobs - JobOrder)/TotalJobs))),
         career_phase = ifelse(trajectory_percentage < 40, "Early", ifelse(trajectory_percentage < 75, "Mid", "Late"))) 
  #select(BGTResId, consolidatedtitleonet, JobDuration, tenure, tenureportion, JobOrder, TotalJobs, trajectory_percentage, career_phase)

hist(tenureportion$trajectory_percentage)
head(tenureportion)
```
