...
for (i in edu_na$inst_low[1:length(edu_na$BGTResID)]) {
  if (i %in% vccs) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("j sargeant reynolds community college",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("j.sargeant reynolds community college",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sargeant.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".john tyler c.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("^john tyler c.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("^dabney.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sargant.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sergant.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sergent.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sargent.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sergeant.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("^sargeant.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("^reynolds.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".reynolds.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".thomas nelson.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl("^thomas nelson.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    ...
    isVCCS <- c(isVCCS,1)
    next
  } else if (grepl(".sargaent.",i, ignore.case = TRUE)) {
    isVCCS <- c(isVCCS,1)
    next
  } else {
    isVCCS <- c(isVCCS,0)
...
edu_na$vccs <- isVCCS
# J sargeant reynolds has a TON of misspellings, will revisit
...
for (i in edu_na$inst_low[1:length(edu_na$BGTResID)]) {
  if (grepl("technical school",i)) {
    ...
    isOther <- c(isOther,1)
    next
  } else if (grepl(".technical center",i)) {
    isOther <- c(isOther,1)
    next
...
for (i in 1:length(edu_na$BGTResID)) {
  if (grepl("high school", edu_na$inst_low[i])) {
    ...
    isHigh <- c(isHigh, 1)
    next
  } else if (grepl("public school", edu_na$inst_low[i])){
    isHigh <- c(isHigh, 1)
    next
