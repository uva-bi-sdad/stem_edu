...
############################################################################################
#Create the no college data set
#no college - 10,860 / 22.75% of the survey
NC<-ATES[ATES$eduattn==4 | ATES$eduattn==5,]; dim(NC); dim(NC)[1]/dim(ATES)[1]

#Recode the kinds of work that need to be split between two major occupation groups
#CNFIELD1 4 and 19
T4<-which(NC$CNFIELD1==4);T4no<-ceiling(length(T4)/2);
NC$CNFIELD1[T4[1:T4no]]<--4
T19<-which(NC$CNFIELD1==19);T19no<-ceiling(length(T19)/2);
NC$CNFIELD1[T19[1:T19no]]<--19;rm(T4,T4no,T19,T19no)
#psfos 3
T3<-which(NC$psfos==3);T3no<-ceiling(length(T3)/2)
NC$psfos[T3[1:T3no]]<--3;rm(T3,T3no)
#wefolp 7
T7<-which(NC$wefolp==7);T7no<-ceiling(length(T7)/2)
NC$wefolp[T7[1:T7no]]<--7;rm(T7,T7no)
...
NCQ9<-ifelse(NC$CNFIELD1==24, "49-0000",
       ifelse(NC$CNFIELD1==23, "47-0000",
        ifelse(NC$CNFIELD1==10, "29-0000",
         ifelse(NC$CNFIELD1==11, "29-0000",
          ifelse(NC$CNFIELD1==1, "17-0000",
           ifelse(NC$CNFIELD1==2, "17-0000",
            ifelse(NC$CNFIELD1==3, "15-0000",
             ifelse(NC$CNFIELD1==4, "15-0000",
              ifelse(NC$CNFIELD1==-4, "19-0000",
               ifelse(NC$CNFIELD1==19, "45-0000",
                ifelse(NC$CNFIELD1==-19, "19-0000",
                 ifelse(NC$CNFIELD1==17, "33-0000",
                  ifelse(NC$CNFIELD1==5, "<5% STW MOG",
                   ifelse(NC$CNFIELD1==6, "<5% STW MOG",
                    ifelse(NC$CNFIELD1==7, "<5% STW MOG",
                     ifelse(NC$CNFIELD1==8, "<5% STW MOG",
                      ifelse(NC$CNFIELD1==16, "<5% STW MOG",
                       ifelse(NC$CNFIELD1==25, "<5% STW MOG",
                        ifelse(NC$CNFIELD1==18, "0% STW MOG",
                         ifelse(NC$CNFIELD1==20, "0% STW MOG",
                          ifelse(NC$CNFIELD1==21, "0% STW MOG",
                           ifelse(NC$CNFIELD1==22, "0% STW MOG",
                            ifelse(NC$CNFIELD1==9, "0% STW MOG",
                             ifelse(NC$CNFIELD1==12, "0% STW MOG",
                              ifelse(NC$CNFIELD1==13, "0% STW MOG",
                               ifelse(NC$CNFIELD1==14, "0% STW MOG",
                                ifelse(NC$CNFIELD1==15, "0% STW MOG",
                                 ifelse(NC$CNFIELD1==26, "OTHER",
                                   ifelse(NC$CNFIELD1==27, "OTHER",
                                    ifelse(NC$CNFIELD1==-8, "Uncodeable",
                                     ifelse(NC$CNFIELD1==-9, "Missing","Valid Skip")))))))))))))))))))))))))))))))
NC$CNFIELD1<-NCQ9
#Q30d. Number of certificates from a community or technical college or other school after HS
NCQ31d<-ifelse(NC$psfos==19, "49-0000",
         ifelse(NC$psfos==7, "47-0000",
          ifelse(NC$psfos==14, "29-0000",
           ifelse(NC$psfos==11, "17-0000",
            ifelse(NC$psfos==18, "51-0000",
             ifelse(NC$psfos==6, "15-0000",
              ifelse(NC$psfos==4, "27-0000",
               ifelse(NC$psfos==3, "45-0000",
                ifelse(NC$psfos==-3, "19-0000",
                 ifelse(NC$psfos==15, "33-0000",
                  ifelse(NC$psfos==5, "<5% STW MOG",
                   ifelse(NC$psfos==1, "<5% STW MOG",
                    ifelse(NC$psfos==16, "<5% STW MOG",
                     ifelse(NC$psfos==9, "<5% STW MOG",
                      ifelse(NC$psfos==20, "<5% STW MOG",
                       ifelse(NC$psfos==2, "<5% STW MOG",
                        ifelse(NC$psfos==10, "0% STW MOG",
                         ifelse(NC$psfos==8, "0% STW MOG",
                          ifelse(NC$psfos==13, "0% STW MOG",
                           ifelse(NC$psfos==12, "0% STW MOG",
                            ifelse(NC$psfos==17, "0% STW MOG",
                             ifelse(NC$psfos==21, "OTHER","Valid Skip"))))))))))))))))))))))
NC$psfos<-NCQ31d
#Q40. Occupation for most work experience programs
NCQ40<-ifelse(NC$wefolp==24, "49-0000",
        ifelse(NC$wefolp==20, "49-0000",
         ifelse(NC$wefolp==1, "47-0000",
          ifelse(NC$wefolp==2, "47-0000",
           ifelse(NC$wefolp==3, "47-0000",
            ifelse(NC$wefolp==4, "47-0000",
             ifelse(NC$wefolp==5, "47-0000",
              ifelse(NC$wefolp==6, "29-0000",
               ifelse(NC$wefolp==7, "29-0000",
                ifelse(NC$wefolp==-7, "31-0000",
                 ifelse(NC$wefolp==14, "17-0000",
                  ifelse(NC$wefolp==18, "51-0000",
                   ifelse(NC$wefolp==21, "51-0000",
                    ifelse(NC$wefolp==11, "15-0000",
                     ifelse(NC$wefolp==26, "27-0000",
                      ifelse(NC$wefolp==16, "33-0000",
                       ifelse(NC$wefolp==19, "<5% STW MOG",
                        ifelse(NC$wefolp==9, "<5% STW MOG",
                         ifelse(NC$wefolp==17, "<5% STW MOG0",
                          ifelse(NC$wefolp==10, "<5% STW MOG",
                           ifelse(NC$wefolp==13, "<5% STW MOG",
                            ifelse(NC$wefolp==22, "0% STW MOG",
                             ifelse(NC$wefolp==23, "0% STW MOG",
                              ifelse(NC$wefolp==8, "0% STW MOG",
                               ifelse(NC$wefolp==12, "0% STW MOG",
                                ifelse(NC$wefolp==15, "0% STW MOG",
                                 ifelse(NC$wefolp==25, "OTHER","Valid Skip")))))))))))))))))))))))))))
NC$wefolp<-NCQ40

#Recode Earnings over the last 12 months Q62
NCQ62<-ifelse(NC$eeearn==1,5000,ifelse(NC$eeearn==2,15000,ifelse(NC$eeearn==3,25000,ifelse(NC$eeearn==4,35000,
         ifelse(NC$eeearn==5,45000,ifelse(NC$eeearn==6,55000,ifelse(NC$eeearn==7,67500,ifelse(NC$eeearn==8,112500,
          ifelse(NC$eeearn==9,175000,NA)))))))))
NC$eeearn<-NCQ62
...
NC<-merge(NC, COC, by.x="empocc", by.y="COC", all.x=TRUE, all.y=FALSE)
...
NCUSE1<-p + theme_grey() + labs(x="", y="") + scale_x_discrete(expand=c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
#  labs(title="Some College No Degree: Usefulness of Certification/License",
#       subtitle="Percent usefulness by skilled technical workforce (STW=714/NonSTW=1,258)") +
  labs(title="",
       subtitle="Some College No Degree\n(STW=714/NonSTW=1,258)") +
...
NCUSE1
ggsave("NCUSE1.pdf", width=6, height=6)
...
T1<-NC[NC$certprog==1,]; dim(T1)  #2582
...
NCUSE2<-p + theme_grey() + labs(x="", y="") + scale_x_discrete(expand=c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
#  labs(title="Some College No Degree: Usefulness of Post-secondary Credential",
#       subtitle="       Percent usefulness by skilled technical workforce (STW=669/NonSTW=1,707).") +
  labs(title="",
       subtitle="Some College No Degree\n(STW=669/NonSTW=1,707)") +
  ...
  theme(legend.position="none",
        ...
        axis.text.y=element_text(size=15, hjust=1.0, colour="#2a2a2b"))
NCUSE2
ggsave("NCUSE2.pdf", width=6, height=6)
...
p<-ggplot(USE, aes(Answer, Category)) +
  geom_tile(aes(fill=Percent), colour="white", lwd=3) +
  #  scale_fill_gradient(low="white", high="steelblue")
  scale_fill_gradient(low="ghostwhite", high="steelblue", limits=c(0,90))

NCUSE3<-p + theme_grey() + labs(x="", y="") + scale_x_discrete(expand=c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
#  labs(title="Some College No Degree: Usefulness of Work Experience Program",
#       subtitle="       Percent usefulness by skilled technical workforce (STW=825/NonSTW=473)") +
  labs(title="",
       subtitle="Some College No Degree\n(STW=825/NonSTW=473)") +
  annotate("text", x=c(rep(c(1,2,3,4),6)),
           y=c(rep(1,4),rep(2,4),rep(3,4),rep(4,4),rep(5,4),rep(6,4)),
  ...
  theme(legend.position="none",
        ...
        axis.text.y=element_text(size=15, hjust=1.0, colour="#2a2a2b"))
NCUSE3
ggsave("NCUSE3.pdf", width=6, height=6)
...
ggplot(data=FINAL) +
  ...
  labs(title="Field of NonDegree Credential by Major Occupation Category and Credential Type",
       subtitle="Some College No Degree (N=6,085)") +
...
###########################################################################
#Earnings from All Jobs over the Past 12 Months
