...
#Divide the ATES data into 3 groups, college degree (CD), some college but no diploma (SC), and HS diploma or less (HS)
#Some college but no degree 15,344 / 32.14% of the survey
SC<-ATES[ATES$eduattn>3 & ATES$eduattn<7,]; dim(SC); dim(SC)[1]/dim(ATES)[1]
#Recode the fields of study that need to be split between two major occupation groups
#CNFIELD1 4 and 19
T4<-which(SC$CNFIELD1==4);T4no<-ceiling(length(T4)/2);
SC$CNFIELD1[T4[1:T4no]]<--4
T19<-which(SC$CNFIELD1==19);T19no<-ceiling(length(T19)/2);
SC$CNFIELD1[T19[1:T19no]]<--19;rm(T4,T4no,T19,T19no)
#psfos 3
T3<-which(SC$psfos==3);T3no<-ceiling(length(T3)/2)
SC$psfos[T3[1:T3no]]<--3;rm(T3,T3no)
#wefolp 7
T7<-which(SC$wefolp==7);T7no<-ceiling(length(T7)/2)
SC$wefolp[T7[1:T7no]]<--7;rm(T7,T7no)

#4-year college degree or higher 18,521 / 38.79% of the survey
...
#HS Degree or less 13879 / 29.07% of the survey
HS<-ATES[ATES$eduattn<4,]; dim(HS); dim(HS)[1]/dim(ATES)[1]
#Recode the fields of study that need to be split between two major occupation groups
#CNFIELD1 4 and 19
...
table(HS$edufos) #fields of study for HS respondents - ALL valid skips
table(SC$edufos) #fields of study for the some college no degree respondents
SCf<-data.frame(table(SC$edufos))[-1,];dim(SCf)
table(CD$edufos) #fields of study for college degree respondents
CDf<-data.frame(table(CD$edufos));dim(CDf)
#Table with the field of study % for CD and SC
Q2<-merge(x=CDf, y=SCf, by="Var1", all=TRUE); names(Q2)<-c("FOS","CD","SC")
Q2$CD<-(Q2$CD/sum(Q2$CD))*100;Q2$SC<-(Q2$SC/sum(Q2$SC, na.rm=TRUE))*100

##############################################################################################
#Q7. Number of certifications/licenses
#Q9. Occupation for most important certifications/licenses
table(SC$cnmain) #3538 currently active certifications/licenses
table(SC$CNFIELD1) #3380 smaller than above - ?? sum(table(SC$CNFIELD1)[6:32])
table(CD$cnmain) #6766 currently active certifications/licenses
table(CD$CNFIELD1) #6598 smaller than above - ?? sum(table(CD$CNFIELD1)[6:32])
table(HS$cnmain) #1440 have at least one certification/license
table(HS$CNFIELD1) #1296 sum(table(HS$CNFIELD1)[6:31])
