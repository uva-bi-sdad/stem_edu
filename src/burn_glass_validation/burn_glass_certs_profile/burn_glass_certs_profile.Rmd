...
cert_query <- "SELECT * FROM \"ads_certs\" WHERE LEFT(\"jobdate\", 7) = \'2017-07\'"
ads_certs_0717 <- as.data.table(dbGetQuery(con, cert_query))
...
```{r dataexplorer}
overall <- melt(DataExplorer::introduce(ads_certs_0717))
colnames(overall) <- c("Metric", "Value")
knitr::kable(overall)
missing <- DataExplorer::profile_missing(ads_certs_0717)
...
dupejobids <- ads_certs_0717 %>% dplyr::group_by(bgtjobid) %>% dplyr::summarise(count = dplyr::n()) %>% dplyr::filter(count > 1)

#Multi-appearance ID by date
dupecount <- ads_certs_0717[bgtjobid %in% dupejobids$bgtjobid, c("bgtjobid", "jobdate")] %>% group_by(bgtjobid, jobdate) %>% summarise(count = n())
...
```{r jobdate hist}
##Job Date
values <-  ads_certs_0717[!is.na(lubridate::as_date(jobdate)), lubridate::as_date(jobdate)]
...
Certification is a text field so let's look for empty and missing strings
```{r cip hist, echo = TRUE, inclue = TRUE}
### Certification 
cert <- ads_certs_0717$certification
sum(is.na(cert))
sum(cert == '')
sum(cert != '')
#looks clean
...
```{r salary hist}
### Salary
values <-  ads_certs_0717[salary >0, salary]
unique_values <- length(unique(values))
frequencies <- as.data.table(table( values))
