...
for (i in colnames(main)){
  evalrow <- tibble::tibble(
    ...
    "Missing" = sum(dataplumbr::is_blank(x = (main[[i]])), na.rm = TRUE) + 
        sum(is.na(x = main[[i]])) 
       , 
    "% Missing" = (sum(dataplumbr::is_blank(x = (main[[i]])), na.rm = TRUE) + 
        sum(is.na(x = main[[i]]))  / 63610), 
    "Invalid" =  sum(grepl(x = (main[[i]]), pattern = "\\bna\\b"), na.rm = TRUE),
    "% Invalid" =  (sum(grepl(x = (main[[i]]), pattern = "\\bna\\b"), na.rm = TRUE) / 63610),
...
bgt_cities <- bgt_cities %>% left_join(cities)
bgt_cities %>% filter(cty)

bgt_cities %>% group_by(city) %>% summarise(count = n()) 


bgt_counties <- as.data.frame(unique(main[, county]) )
colnames(bgt_counties) <- c("counties")

bgt_counties <- bgt_counties %>% left_join(counties)

bgt_counties %>% filter(is.na(cty)) %>% mutate(county_minus = stringr::str_remove(counties, "\\scity|\\sCity")) %>% left_join(cities, by = c("county_minus" = "counties")) %>% filter(is.na(city))

cities %>% filter(grepl(counties, "Manassas") == TRUE)

unique(main[, fips])
bgt_fips_cty <- as.data.frame((unique(main[, fipscounty])))
colnames(bgt_fips_cty) <- c("fipscty")
bgt_fips_cty$fipscty <- as.character(bgt_fips_cty$fipscty)
bgt_fips_cty <- bgt_fips_cty %>% 
  mutate(fipscounty_short = ifelse(nchar(fipscty) == 3, fipscty, 
                             ifelse(nchar(fipscty) == 2, stringr::str_pad(fipscty, width = 3, pad = "0"), 
                             stringr::str_pad(fipscty, width = 3, pad = "0")))) %>%
  mutate(fipscounty = paste0("51", fipscounty_short))
...
bgt_fips_cty <- bgt_fips_cty %>% left_join(fipscounties, by = c("fipscounty" = "fipscty"))

main[1:6, c(fips,fipscounty)]

bgt_fips <- as.data.frame(unique(main[, fips]))
colnames(bgt_fips) <- c("fips")
bgt_fips

bgt_fips$fips <- as.factor(bgt_fips$fips)
bgt_fips <- bgt_fips %>% left_join(fipscounties, by = c("fips" = "fipscty"))

bgt_fips %>% filter(is.na(fc))
bgt_fips_cty %>% filter(is.na(fc))
###

arrange(main[duplicated(main[,2:13])], jobdate, occfam, occfamname, employer, city, state, county, fipsstate, fipscounty, fips, lat, lon)

main

main[,2:13] %>% distinct() %>% arrange(jobdate, occfam, occfamname, employer, city, state, county, fipsstate, fipscounty, fips, lat, lon)

duplicates <- main %>% 
  group_by(jobdate, occfam, occfamname, employer, city, state, county, fipsstate, fipscounty, fips, lat, lon) %>% 
  summarise(count = n()) %>% 
  filter(count > 1) %>% 
  arrange(jobdate, occfam, occfamname, employer, city, state, county, fipsstate, fipscounty, fips, lat, lon) 

sum(duplicates$count)
min(duplicates$count)
max(duplicates$count)

duplicates %>% filter(count > 30)

main_orig[duplicated(main_orig),]

skills_17 <- read.csv(unzip("~/git/stem_edu/data/stem_edu/original/Burning_Glass_Data/Skill/2017/Skills_2017-07.zip"), sep = "\t")



head(skills_17)
skills_17$BGTJobId <- as.character(skills_17$BGTJobId)
head(main)
...
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$rawdata_id)), na.rm = FALSE)) / nrow(OJ2)
(nrow(OJ2) - sum(is.na(OJ2$rawdata_id), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(grepl(x = (OJ2$rawdata_id), pattern = "\\bna\\b"), na.rm = TRUE)) / nrow(OJ2)
length(unique(OJ2$rawdata_id))

(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$normalizedTitle_onetCode)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(is.na(OJ2$normalizedTitle_onetCode), na.rm = TRUE))/nrow(OJ2)
(nrow(OJ2) - sum(grepl(x = (OJ2$normalizedTitle_onetCode), pattern = "\\bna\\b"), na.rm = TRUE)) / nrow(OJ2)
length(unique(OJ2$normalizedTitle_onetCode))

(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$normalizedTitle_onetName)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(is.na(OJ2$normalizedTitle_onetName), na.rm = TRUE))/nrow(OJ2)
(nrow(OJ2) - sum(grepl(x = (OJ2$normalizedTitle_onetName), pattern = "\\bna\\b"), na.rm = TRUE)) / nrow(OJ2)
length(unique(OJ2$normalizedTitle_onetName))

(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$datePosted)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(is.na(OJ2$datePosted), na.rm = TRUE))/nrow(OJ2)
(nrow(OJ2) - sum(grepl(x = (OJ2$datePosted), pattern = "\\bna\\b"), na.rm = TRUE)) / nrow(OJ2)
length(unique(OJ2$datePosted))

(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$responsibilities)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$experienceRequirements)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$jobDescription)), na.rm = TRUE)) / nrow(OJ2)

(nrow(OJ2) - sum(dataplumbr::is_blank(x = (OJ2$hiringOrg)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(is.na(OJ2$hiringOrg), na.rm = TRUE))/nrow(OJ2)
(nrow(OJ2) - sum(grepl(x = (OJ2$hiringOrg), pattern = "\\bna\\b"), na.rm = TRUE)) / nrow(OJ2)
length(unique(OJ2$hiringOrg))

length(unique(OJ2$rawdata_id))
max(nchar(unique(OJ2$rawdata_id)))
min(nchar(unique(OJ2$rawdata_id)))

coords <- as.data.frame(st_coordinates(OJ2))
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (coords$X)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (coords$X)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (coords$X)), na.rm = TRUE)) / nrow(OJ2)

(nrow(OJ2) - sum(dataplumbr::is_blank(x = (coords$Y)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (coords$Y)), na.rm = TRUE)) / nrow(OJ2)
(nrow(OJ2) - sum(dataplumbr::is_blank(x = (coords$Y)), na.rm = TRUE)) / nrow(OJ2)

length(unique(coords$X))
length(unique(coords$Y))

NULL -> st_geometry(OJ2)
OJ2[duplicated(OJ2),]
sum(duplicated(OJ2))
colnames(OJ2)
OJ2 %>% group_by(normalizedTitle_onetCode, normalizedTitle_onetName, datePosted, hiringOrg) %>% summarise(count = n()) %>% filter(count == 1)

unique(OJ2$datePosted)
checkdupes <- OJ2[,c(2:8)]

OJ2[duplicated(OJ2[,c(2:8, 11)]),]
```
...
BGT <- BG_join_point %>% group_by(occfam, occfamname, Region) %>% summarise(count = n())
st_geometry(BGT) <- NULL
OpenJobs <- readRDS("data/stem_edu/working/BGexplorevalidate/BG_Shapefiles/openjobs_point_aggregated_0717_fortable.RDS")

BGT
OpenJobs

thing <- BGT %>% left_join(OpenJobs, by = c("occfam", "Region"))
thing <- thing %>% mutate(OCC = paste(occfam, occfamname)) 
thing <- thing[,3:6]
colnames(thing) <- c("Region", "BGT Count", "OJ Count", "OccFam")

thing2 <- dcast.data.table(as.data.table(thing), OccFam ~ Region, value.var=list('BGT Count','OJ Count'))

thing2[is.na(thing2)] <- 0

thing3 <- thing2 %>% transmute(
  OccFam = OccFam,
  "Region NA" = `BGT Count_NA` - `OJ Count_NA`, 
  "Region 1" = `BGT Count_1` - `OJ Count_1`, 
  "Region 2" = `BGT Count_2` - `OJ Count_2`, 
  "Region 3" = `BGT Count_3` - `OJ Count_3`, 
  "Region 5" = `BGT Count_5` - `OJ Count_5`, 
  "Region 4" = `BGT Count_4` - `OJ Count_4`, 
  "Region 6" = `BGT Count_6` - `OJ Count_6`, 
  "Region 7" = `BGT Count_7` - `OJ Count_7`, 
  "Region 8" = `BGT Count_8` - `OJ Count_8`,
  "Region 9" = `BGT Count_9` - `OJ Count_9`
)
write.csv(thing3, "~/git/stem_edu/data/stem_edu/working/BGexplorevalidate/BG_Shapefiles/vicki_table.csv")
```
