...
main2 %>% st_set_crs(st_crs(virginia))
...
duplication <- tibble("Dataset" = c("BGT", "Open Jobs"), 
       ...
       "# Dupes" = c(
        ...
        "Percent Unuplicated" = c(
          ...
          ( nrow(OJ2[!duplicated(OJ2[,2:8]),])) / nrow(main) ),
...
oj_perc_comp <- rbind(oj_id, onet, onetname, date)
...
BGT_OJ_agg <- BGT_OJ_agg %>% 
  mutate(diff = (BGT - OJ), 
         per_diff = ((BGT - OJ) / ((BGT + OJ)/2)),  
         perc_diff = 100*per_diff, 
         diffandpercdiff = paste(per_diff, " / ", perc_diff)) 
  

BGT_OJ_agg_ <- dcast.data.table(as.data.table(BGT_OJ_agg), occ_fam ~ econ_region, value.var=list('perc_diff'))
