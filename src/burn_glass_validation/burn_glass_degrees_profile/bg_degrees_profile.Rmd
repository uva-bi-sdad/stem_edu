...
deg_query <- "SELECT * FROM \"ads_degrees\" WHERE LEFT(\"jobdate\", 7) = \'2017-07\'"
ads_deg_0717 <- as.data.table(dbGetQuery(con, deg_query))
...
```{r dataexplorer}
overall <- melt(DataExplorer::introduce(ads_deg_0717))
colnames(overall) <- c("Metric", "Value")
knitr::kable(overall)
missing <- DataExplorer::profile_missing(ads_deg_0717)
...
dupejobids <- ads_deg_0717 %>% dplyr::group_by(bgtjobid) %>% dplyr::summarise(count = dplyr::n()) %>% dplyr::filter(count > 1)

#Multi-appearance ID by date
dupecount <- ads_deg_0717[bgtjobid %in% dupejobids$bgtjobid, c("bgtjobid", "jobdate")] %>% group_by(bgtjobid, jobdate) %>% summarise(count = n())
...
```{r jobdate hist}
##Job Date
values <-  ads_deg_0717[!is.na(lubridate::as_date(jobdate)), lubridate::as_date(jobdate)]
...
```{r}
### Degree
values <-  ads_deg_0717[!is.na(as.numeric(degreelevel)), as.numeric(degreelevel)]
unique_values <- length(unique(values))
frequencies <- as.data.table(table( values))
ggplot(frequencies, aes(as.factor(values), N)) +
  geom_bar(stat = "identity", fill ="paleturquoise4", width=.7) +
  ggtitle(paste("Degree Level", "Value Distribution")) +
  xlab("Degree Level") +
  ylab("N") +
  theme(panel.background=element_blank(), axis.text.x = element_text(angle = 90, hjust = 1))
```
...
```{r salary hist}
### Salary
values <-  ads_deg_0717[salary >0, salary]
unique_values <- length(unique(values))
frequencies <- as.data.table(table( values))
